...
import { AngelApiService } from './api.service';

@Injectable({
  providedIn: 'root',
})
export class AngelApiInterceptor implements HttpInterceptor {
  /**
   * Constructor
   */
  constructor(
    @Inject(ANGEL_API_DEFAULT_DELAY) private _defaultDelay: number,
    private _angelApiService: AngelApiService
  ) {}
  ...
  intercept(
    ...
    const { handler, urlParams } = this._angelApiService.findHandler(

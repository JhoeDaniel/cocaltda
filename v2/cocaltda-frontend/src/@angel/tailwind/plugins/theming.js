...
const generateVariableColors = (theme) => {
    ...
    const customPropertiesWithOpacity = (name) => ({
        ...
        if (opacityValue) {
            return `rgba(var(--angel-${name}-rgb), ${opacityValue})`;
        }
        if (opacityVariable) {
            return `rgba(var(--angel-${name}-rgb), var(${opacityVariable}, 1))`;
        }
        return `rgb(var(--angel-${name}-rgb))`;
    };
...
const theming = plugin.withOptions((options) => ({
    ...
    const mapVariableColors = _.fromPairs(_.map(options.themes, (theme, themeName) => [
        ...
        ))), (value, key) => [[`--angel-${e(key)}`, value], [`--angel-${e(key)}-rgb`, chroma(value).rgb().join(',')]])))
    ]));
    ...
    const schemeCustomProps = _.map(['light', 'dark'], (colorScheme) => {
        ...
        const background = theme(`angel.customProps.background.${colorScheme}`);
        const foreground = theme(`angel.customProps.foreground.${colorScheme}`);
        ...
        return {
            [(isDark ? darkSchemeSelectors : lightSchemeSelectors)]: {
                ...
                ..._.fromPairs(_.flatten(_.map(background, (value, key) => [[`--angel-${e(key)}`, value], [`--angel-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
                ..._.fromPairs(_.flatten(_.map(foreground, (value, key) => [[`--angel-${e(key)}`, value], [`--angel-${e(key)}-rgb`, chroma(value).rgb().join(',')]])))
            }
        };
    });
